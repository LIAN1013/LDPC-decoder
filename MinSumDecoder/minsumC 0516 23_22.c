...
#include <string.h>
/*
    5/16 22:30 add third.minBetaValue, s_tilde function   --Jade
    5/16 23:14
*/
/**
    ...
    V �@���HH�x�}�u���A��Ū��H�x�}
    X �ھ�H�x�}�ͦ�G�x�}
    X �ھ�G�x�}�ͦ��h�զX�kcodeword
    V �ھڦX�kcodeword�ͦ��h���g�LAWGN�q�D�����ո�Ty(�q�D���T�ܲ��Ƥ��P)
    V �ھ�y�ѽX�A�ˬd�ѽX�O�_���\
    X �p��ɶ��Ĳv

**/


/*  ���ڼg�@��Ū��Alist�A�qcodeword=1010101010.....10101010�@�s����U�@��(+1)�}�l�w�ѡA�ͦ��@�ӫD��0�X�kcodeword��C�P���{��      */
...
{
    char filename[100]={"GallagerH.txt"};
...
void Generate_TestData(const int count, int N, int M, double EbN0_dB) {
    char output_name[] = "test_data.txt";
    ...
    for (int c = 0; c < count; c++) {
        // --- 1. �ͦ� Codeword (�o�̼ȥΥ��s�A�Y�� G �x�}�i�אּ�H���X) ---
        // ���ܡG�Y�n���դ��P�X�r�A�i�H���ͦ��줸�A�����T�O���� Hc^T = 0
        for (int i = 0; i < N; i++) {
        ...
        // --- 3. �ͦ��L�q�D�᪺ y (BPSK: 0 -> +1.0, 1 -> -1.0) ---
        for (int i = 0; i < N; i++) {
...
double s_tilde( double x, double y){
    const double c = 0.5;
    if(fabs(x + y) < 2 && fabs(x - y) > 2*fabs(x + y)) return c;
    else if(fabs(x - y) < 2 && fabs(x + y) > 2*fabs(x - y)) return -c;
    else return 0.0;
}
...
bool isDecodeSucess(double* channel_sigma,int* decoder_iteration,int decoder_max_iteration, double* LLR_Current)
{
    ...
    double* LLR_FinalResult=(double*)malloc(N*sizeof(double));  // Store rji
    int* Ci=(int*)malloc(N*sizeof(int));;                       // Final Decision

    *decoder_iteration=0;

    int for_test_only_true_time=0;
    ...
    while(*decoder_iteration<decoder_max_iteration)
    {
        ...
        for(int i=0;i<M;i++)
        {
            ...
            third.minBetaValue = 1e100;

            // �Ĥ@�M�M���C�@row�A���έp�C�@row�����t����m�M�̤p�ȻP���p�ȼƭȤj�p�P��m
            ...
            for(int j=0;j<H_rows[i].row_weight;j++)// EX 2,3,4,5 in Hamming code first line
            {
                ...
                else if(tempValue<second.minBetaValue)
                {
                    ...
                    third.minBetaPosition=H_rows[i].row_place[j];
            ...
            // �ĤG�M�M���p��X�Ω�̲׵��G���^�ǭ�(�H�`�t���M�`phi�ƭȭp��)�M�~�򭡥N��LLR(�H�`�t�������Ӧ�m�t���ƶq�P�_���t�A�H�`phi�Ȧ����Ӧ�mphi�ƭȧP�_�ƭȤj�p)
            ...
            for(int j=0; j<H_rows[i].row_weight; j++){
                ...
                if(tempPosition == first.minBetaPosition){
                    x = second.minBetaValue; y = third.minBetaValue;
                 }
                else if(tempPosition == second.minBetaPosition){
                    x = first.minBetaValue;  y = third.minBetaValue;
                }
                else{
                    x = first.minBetaValue;  y = second.minBetaValue;
                }

                // 在這裡宣告並計算，每次循環都是獨立乾淨的
                double delta = s_tilde(x, y);
                double correctionValue = x + delta;

                if (correctionValue < 0.0) correctionValue = 0.0;

                ReturnTotalExcpetI[tempPosition] += alpha * isNegativeI[j] * correctionValue;
                ReturnTotal[tempPosition] += alpha * isNegativeI[j] * correctionValue;
        ...
        if(isAllZero==true){
            ...
            printf("T:%10ld F:%10ld\n",GLOBAL_TRUEBIT,GLOBAL_FALSEBIT);
            for_test_only_true_time++;
            if(for_test_only_true_time>for_test_only_true_time_max)
                return true;
        }
        else{
...
int main(void)
{
    ...
    const int deocder_iteration_max = 100;
    ...
    // open test data file
    char filename[100]={"test_data.txt"};
    FILE *TEST_DATA=fopen(filename,"r");
    if(!TEST_DATA){
        printf("[SYSTEM]MAIN_Open test data file failed\n");
        return false;
    }
    printf("sucessfully read test data\n");
