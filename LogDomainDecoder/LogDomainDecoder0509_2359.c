...
double* Read_Channel_Output(double* channel_output)
{
    //char filename[100]={"C:\\Users\\09043\\OneDrive\\Python\\LDPC\\HammingCodeEX.txt"};
    char filename[100]={"Channel_Output.txt"};//相對路徑，放在專案資料夾內
    FILE *fp=fopen(filename,"r");
    ...
    for(int i=0;i<N;i++)
    {
        fscanf(fp,"%lf",&channel_output[i]);
        printf("%lf",channel_output[i]);
    }
...
double phi(double x)
{
    ...
    /*gemini給的算法(感覺這樣算合理，但還是給你們確認後再改)
    // 直接計算，x 在外部應保證為正數
...
int main(void)
{
    ...
    double channel_sigma=sqrt(0.5);
    int decoder_iteration=0;
    const int decoder_max_iteration=2;
    ...
    channel_output=Read_Channel_Output(channel_output);
    //channel_output={-1.5,0.8,-0.9,0.7,0.5,-1.1,-0.4,-1.2};
