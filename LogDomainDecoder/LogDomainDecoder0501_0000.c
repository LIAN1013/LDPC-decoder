...
int N=-1,K=-1,M=-1;    // H(7,4) hamming code
RowInfo *H_rows=NULL;
ColInfo *H_cols=NULL;
...
double phi(double x)
{
    if(x<0){printf("_______________________ERROR!_________________________\n");system("pause");}
    ...
    while(decoder_iteration<decoder_max_iteration)
    {
        ...
        for(int i=0;i<M;i++)//EX 3 iterations for hamming code
        {
            ...
            for(int j=0;j<H_rows[i].row_weight;j++)
            {
                ...
                printf("%d ReturnValue->%llf ReturnValueExcept%d->%llf\n",H_rows[i].row_place[j],returnValue,H_rows[i].row_place[j],returnValueExceptI);
            }
            for(int j=0;j<N;j++)
            {
                printf("J=%d TOTAL:%llf  TOTALEXCEPT:%llf\n",j,ReturnTotal[j],ReturnTotalExceptI[j]);
            }
            printf("\n");
        }
